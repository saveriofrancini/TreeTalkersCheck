...
# battery calculation
batteryLevel <- 2*1100*dbTT_Today$adc_vbat/dbTT_Today$adc_bandgap/1000
...
lowBatteryDB <- aggregate(lowBatteryDB[,2:3], list(as.character(lowBatteryDB$TTid)), min)
...
dbTT_Today$tref_0 <- FromDigitalNumberToTemperature(dbTT_Today$tref_0)
dbTT_Today$tref_1 <- FromDigitalNumberToTemperature(dbTT_Today$tref_1)
dbTT_Today$theat_0 <- FromDigitalNumberToTemperature(dbTT_Today$theat_0)
dbTT_Today$theat_1 <- FromDigitalNumberToTemperature(dbTT_Today$theat_1)
...
DeltaT_max <- aggregate(DeltaTon, list(as.character(dbTT_Today$TTid)), max)
...
DeltaTMax <- as.character(DeltaT_max$DeltaTMax)[match(as.character(dbTT_Today$TTid), as.character(DeltaT_max$Location))]
dbTT_Today <- cbind(dbTT_Today, DeltaTMax)
sapFluxDensity <- (118.99*(((as.double(as.character(dbTT_Today$DeltaTMax))/
                               (as.double(as.character(dbTT_Today$DeltaTon))-
                                  as.double(as.character(dbTT_Today$DeltaToff))))-1)^1.231))
...
stemDiameterVariation <- (0.000000008*(dbTT_Today$growth^2))-(0.0016*dbTT_Today$growth)+89.032
...
write.table(lowBatteryDB, file.path(out_dir, paste0(whichDay, "_TTAlert.csv")), row.names = FALSE, sep = ";")
write.table(dbTT_Today, file.path(out_dir, paste0(whichDay, "_TT.csv")), row.names = FALSE, sep = ";")
