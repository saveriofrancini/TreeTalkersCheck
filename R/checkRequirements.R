...
checkRequirements <- function(whichDay = Sys.Date() - 1) {
  ...
  Rdir <-
    list.files(file.path("C:", "Program Files", "R"), full.names = T)
  ...
  writeLines(as.character(whichDay),
             file.path(tempdir(), "whichDay.txt"))

  # Loading required packages
  required_packages <- c("lubridate", "ggplot2", "grid", "knitr", "rmarkdown", "data.table")
  missing_packages <-
