...
Plot <- function(whichDay = Sys.Date() - 1) {
  outPathReport <-
    file.path(file.path("C:", "TreeTalkerDB"), "dailyReports")
  ...
  outPath <-
    file.path(TTdirectory, "dailyReports", paste0(whichDay, "_plots"))
  ...
  TTdb_today <-
    read.table(
      file.path(TTdirectory, "dailyDB", paste0(whichDay, "_TT.csv")),
      header = T,
      row.names = NULL,
      sep = ";"
    )
  TTdb_today$timeStamp <-
    strftime(as.POSIXct(as.character(TTdb_today$timeStamp)), format = "%H")
  ...
  ggplot(TTdb_today,
         aes(timeStamp, temperature, colour = Location.x),
         fill = TTid) +
    geom_point(size = 0.8) +
  ...
  ggsave(file.path(outPath, "temperature.PNG"),
         width = 10,
         height = 6)
  ...
  ggplot(TTdb_today,
         aes(timeStamp, batteryLevel, colour = Location.x),
         fill = TTid) +
    geom_point(size = 0.8) +
    xlab("Hour of the day") +
  ...
  ggsave(file.path(outPath, "batteryLevel.PNG"),
         width = 10,
         height = 6)
  ...
  for (sito_i in siti) {
    ...
    ggsave(file.path(outPath, paste0("temperature", sito_i, ".PNG")),
           width = 10,
           height = 6)
    ...
    ggsave(file.path(outPath, paste0("sapFluxDensity", sito_i, ".PNG")),
           width = 10,
           height = 6)
    ...
    ggsave(file.path(outPath, paste0("batteryLevel", sito_i, ".PNG")),
           width = 10,
           height = 6)
  ...
  if (dir.exists(outPiante) == F)
    dir.create(outPiante)

  piante <- as.character(unique(TTdb_today$TTid))
  piante <- na.omit(piante)
  ...
  for (pianta_i in piante) {
    ...
    stemDiameterVariationMin <- min(db_pianta_i$stemDiameterVariation, na.rm = T)
    stemDiameterVariationMax <- max(db_pianta_i$stemDiameterVariation, na.rm = T)
    ...
    db_pianta_i$stemDiameterVariation <-
      scaleFUN(db_pianta_i$stemDiameterVariation)

    if (stemDiameterVariationMin == stemDiameterVariationMax) {
      db_pianta_i$stemDiameterVariation <-
        rep(0.5, length(db_pianta_i$stemDiameterVariation))
    }

    #plot

    ggplot(db_pianta_i,
           aes(x = timeStamp, y = stemDiameterVariation, group = TTid)) +
      geom_line() +
      geom_point(size = 2.2) +
      ...
      annotation_custom(grobTree(textGrob(
        paste0("Min = ", round(stemDiameterVariationMin, 3)),
        x = 0.25,
        y = 0.2,
        gp = gpar(col = "darkblue", fontsize =
                    30)
      ))) +
      annotation_custom(grobTree(textGrob(
        paste0("Max = ", round(stemDiameterVariationMax, 3)),
        x = 0.25,
        y = 0.1,
        gp = gpar(col = "darkblue", fontsize =
                    30)
    ...
    ggsave(file.path(
      outPiante,
      paste0("stemDiameterVariation", pianta_i, ".PNG")
    ),
    width = 10,
    height = 6)
  }
  ...
  cloud_db_today <-
    read.table(
      file.path(TTdirectory, "dailyDB", paste0(whichDay, "_cloud.csv")),
      header = T,
      row.names = NULL,
      sep = ";"
    )
  cloud_db_today$timeStamp <-
    strftime(as.POSIXct(as.character(cloud_db_today$timeStamp)), format = "%H")

  # Battery trend in the different sites

  ggplot(cloud_db_today,
         aes(timeStamp, Battery, colour = Location.x),
         fill = Location.x) +
    geom_point(size = 2) +
    xlab("Hour of the day") +
  ...
  ggsave(file.path(outPath, "cloudBattery.PNG"),
         width = 10,
         height = 6)
