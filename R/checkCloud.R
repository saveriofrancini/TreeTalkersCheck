...
# dbCloudToday
dbCloud$timeStamp <- lubridate::as_datetime(dbCloud$timeStamp)+7200
...
colonneFullZero <- apply(dbCloudToday, 2, function(x){any(x != 0)})
...
condition_2 <- dbCloudToday$pendingRecords < maximumNumberOfPengindItems
condition_3 <- dbCloudToday$Battery > minimumBattery
lowSignalSensorsList <- list()
for (i in 1:nrow(dbCloudToday)) {
  Sensors_row_i <- as.double(as.character(dbCloudToday[i,16:ncol(dbCloudToday)]))
  lowSignalSensors <- Sensors_row_i < minimumTalkerSignal
  lowSignalSensors[is.na(lowSignalSensors)] <- FALSE
  whichSensorsLowSignal <- (1:(ncol(dbCloudToday)-14))[lowSignalSensors]
...
# RSSI signal
lowSignalSensorsList <- lapply(lowSignalSensorsList, function(x){paste(x, collapse = ";")})
...
lowSignalSensorsDb <- cbind(lowSignalSensorsDb, lowSignalSensors[lowSignalSensors!=""])
...
tipology <- c(rep("cloudSignal", nrow(segnale)),
...
write.table(out, file.path(out_dir, paste0(whichDay, "_cloudAlert.csv")), row.names = FALSE, sep = ";")
write.table(dbCloudToday, file.path(out_dir, paste0(whichDay, "_cloud.csv")), row.names = FALSE, sep = ";")
